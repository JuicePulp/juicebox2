...
export function initPairModal() {
  ...
  if (codeWrap)
    codeWrap.addEventListener("click", handleCopyCode);
  checkPairingStatus();
  updateDeviceStatus();
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(devices) && devices.length > 0) {
          ...
          try {
            localStorage.setItem("juicebox_paired", "true");
          } catch {}
          try {
            localStorage.setItem("juicebox_device_id", device.device_id);
          } catch {}
          try {
            localStorage.setItem("juicebox_device_name", device.device_name);
          } catch {}
        }
      }
    } catch {}
...
function showPairedState(deviceName, pairedAt) {
